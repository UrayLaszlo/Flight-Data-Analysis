...
# Question 4
# Grouping the data by delay at origin
...
# Create binary column by arrival delay and convert it to factor
first_year$target <- as.numeric(first_year$ArrDelay > 0)
first_year$target <- factor(first_year$target)
...
# Log. regression classification model
learner_log_r <- lrn("classif.log_reg")
gc_log_r <- po('imputemean') %>>%
  po(learner_log_r)
graph_lrn_lr <- GraphLearner$new(gc_log_r)
graph_lrn_lr$train(task, row_ids = train_set)
graph_lrn_lr$predict(task, row_ids = test_set)$score() 
